...
#Preparing_dataset
train = fread("ADNI_corrected.txt")
test = fread("ANM1_corrected.txt")
...
train = expr.adni
test = expr.anm1
train1 = PTAT(expression.set = train, demographic.set = adni, deg_adni)
test1 = PTAT(expression.set = test, demographic.set = anm1, deg_adni)
...
#TF (SVM, RF)
train1 = PTAT(expression.set = train, demographic.set = adni, feature = tf_adni)
test1 = PTAT(expression.set = test, demographic.set = anm1, feature = tf_adni)
...
#CFG (SVM, RF)
train1 = PTAT(expression.set = train, demographic.set = adni, feature = cfg_adni)
test1 = PTAT(expression.set = test, demographic.set = anm1, feature = cfg_adni)
...
#DEG (SVM, RF)
test1 = PTAT(expression.set = expr.anm2, demographic.set = anm2, feature = deg_adni)
pred = predict(SVM_DEG, test1,  decision.values = TRUE, probability = TRUE)
prob = attr(pred, "probabilities")[,2]
fg = prob[test1$dx == 1]
bg = prob[test1$dx == 0]
...
#TF (SVM, RF)
test1 = PTAT(expression.set = expr.anm2, demographic.set = anm2, feature = tf_adni)
pred = predict(SVM_TF, test1,  decision.values = TRUE, probability = TRUE)
prob = attr(pred, "probabilities")[,2]
fg = prob[test1$dx == 1]
bg = prob[test1$dx == 0]
...
#CFG (SVM, RF)
test1 = PTAT(expression.set = expr.anm2, demographic.set = anm2, feature = cfg_adni)
SVM_CFG = svm(as.factor(dx) ~ ., data=train1, kernel="radial", probability = TRUE)
pred = predict(SVM_CFG, test1,  decision.values = TRUE, probability = TRUE)
prob = attr(pred, "probabilities")[,2]
fg = prob[test1$dx == 1]
