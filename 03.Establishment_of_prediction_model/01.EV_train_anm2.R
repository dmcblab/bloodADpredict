...
#Preparing_dataset
train = fread("ANM2_corrected.txt")
...
train = expr.anm2
test = expr.adni
train1 = PTAT(expression.set = train, demographic.set = anm2, deg_anm2)
test1 = PTAT(expression.set = test, demographic.set = adni, deg_anm2)
...
#TF (SVM, RF)
train1 = PTAT(expression.set = train, demographic.set = anm2, feature = tf_anm2)
test1 = PTAT(expression.set = test, demographic.set = adni, feature = tf_anm2)
...
#CFG (SVM, RF)
train1 = PTAT(expression.set = train, demographic.set = anm2, feature = cfg_anm2)
test1 = PTAT(expression.set = test, demographic.set = adni, feature = cfg_anm2)
...
#Preparing_dataset
test = fread("ANM1_corrected.txt"); test = data.frame(test, stringsAsFactors = F)
rownames(test) = test$sample
...
#DEG (SVM, RF)
test1 = PTAT(expression.set = expr.anm1, demographic.set = anm1, feature = deg_anm2)
pred = predict(SVM_DEG, test1,  decision.values = TRUE, probability = TRUE)
prob = attr(pred, "probabilities")[,2]
fg = prob[test1$dx == 1]
bg = prob[test1$dx == 0]
...
#TF (SVM, RF)
test1 = PTAT(expression.set = expr.anm1, demographic.set = anm1, feature = tf_anm2)
pred = predict(SVM_TF, test1,  decision.values = TRUE, probability = TRUE)
prob = attr(pred, "probabilities")[,2]
fg = prob[test1$dx == 1]
bg = prob[test1$dx == 0]
...
#CFG (SVM, RF)
test1 = PTAT(expression.set = expr.anm1, demographic.set = anm1, feature = cfg_anm2)
SVM_CFG = svm(as.factor(dx) ~ ., data=train1, kernel="radial", probability = TRUE)
pred = predict(SVM_CFG, test1,  decision.values = TRUE, probability = TRUE)
prob = attr(pred, "probabilities")[,2]
fg = prob[test1$dx == 1]
